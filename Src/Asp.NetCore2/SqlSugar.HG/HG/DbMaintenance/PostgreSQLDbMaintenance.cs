        ...
        protected override string GetCreateTableSql(string tableName, List<DbColumnInfo> columns)
        {
            ...
            foreach (var item in columns)
            {
                ...
                if (item.IsIdentity)
                {
                    ...
                    {
                        string identityDataType;
                        if (lowerDataType == "int8" || lowerDataType == "long" || lowerDataType == "bigint")
                        {
                            identityDataType = "INT8 GENERATED BY DEFAULT AS IDENTITY";
                        }
                        else if (lowerDataType == "int2" || lowerDataType == "smallint" || lowerDataType == "short")
                        {
                            identityDataType = "INT2 GENERATED BY DEFAULT AS IDENTITY";
                        }
                        else
                        {
                            identityDataType = "INT4 GENERATED BY DEFAULT AS IDENTITY";
                        }
